...
//        return null;
    ...
    public void addCredit(int i, Person person) {
        int a;
        qst = "select balance from customer where email = '"+ person.getEmail() +"'";

        try {
            rs = connection.prepareStatement(qst).executeQuery();
            rs.next();
            a = Integer.parseInt(rs.getString(1));
            a = a+1;

            qst = "update customers set balance = '"+a+"' where email = '"+person.getEmail()+"'";
            connection.prepareStatement(qst).executeUpdate();
